...
#ifndef XMODBUSTCPCLIENT_P_H
#define XMODBUSTCPCLIENT_P_H
...
class XModbusTcpClientPrivate : public QModbusClientPrivate
{
    Q_DECLARE_PUBLIC(XModbusTcpClient)

public:
    void setupTcpSocket()
    {
        Q_Q(XModbusTcpClient);
                                 ...
                                 QModbusReply::ReplyType type) override
    {
        auto writeToSocket = [this](quint16 tId, const QModbusRequest &request, int address) {
            ...
            if (writtenBytes == -1 || writtenBytes < buffer.size()) {
                ...
                q->setError(XModbusTcpClient::tr("Could not write request to socket."),
                            QModbusDevice::WriteError);
                return false;
            }
            bool isFlushed = m_socket->flush();
            qCDebug(QT_MODBUS_LOW) << "(TCP client) buffer flushed is " << isFlushed;
