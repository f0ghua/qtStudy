...
private:
    void updateSignalsNameByAttribute();
    Status loadDbcFile(const QString &fileName);
    Status loadFibexFile(const QString &fileName);
    Status xmlParseFibex(const QDomElement &element);
    Status xmlParseProject(const QDomElement &element);
    Status xmlParseElements(const QDomElement &element);
