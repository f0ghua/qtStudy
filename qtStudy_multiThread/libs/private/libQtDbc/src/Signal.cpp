...
double Signal::minimumPhyValue(bool isOutputDirectly) const
{
    if ((minimumPhysicalValue) || (isOutputDirectly))
		return minimumPhysicalValue;
	else
		return rawToPhysicalValue(minimumRawValue());
}

double Signal::maximumPhyValue(bool isOutputDirectly) const
{
    if ((maximumPhysicalValue) || (isOutputDirectly))
...
double Signal::maximumRawValue() const
{
    ...
    switch (valueType) {
    	...
    	case ValueType::Signed:
        	if (valueType == ValueType::Signed) {
                maximumRawValue = (bitSize<2)?0:(2<<(bitSize-2))-1;
        	} else {
                maximumRawValue = (bitSize<1)?0:(2<<(bitSize-1))-1;
        	}
        	break;
...
void Signal::encodePhy(uint8_t *msgData, double phyValue) const
{
