...
void File::updateSignalsNameByAttribute()
{
    QMap<unsigned int, Message>::iterator it;

    for (it = m_pNetwork->messages.begin();
         it != m_pNetwork->messages.end();
         it++) {

        Message &msg = it.value();
        QMap<QString, Signal> changedSignals;

        QMap<QString, Signal>::iterator is;
        for (is = msg.m_signals.begin();
             is != msg.m_signals.end();) {
            const QString sigName = is.key();
            Signal &sig = is.value();

            const QString attrName = "SystemSignalLongSymbol";
            QString longName =
                    DBCHelper::getAttributeValueString(&sig, m_pNetwork, attrName, NULL);
            if (!longName.isEmpty()) {
                qDebug() << QObject::tr("signal %1 has longName %2").arg(sigName).arg(longName);
                sig.name = longName;
                changedSignals.insert(longName, sig);

                is = msg.m_signals.erase(is);
            } else {
                is++;
            }
        }

        for (is = changedSignals.begin();
             is != changedSignals.end();
             is++) {
            msg.m_signals.insert(is.key(), is.value());
        }
    }
}
...
Status File::loadDbcFile(const QString &fileName)
{
