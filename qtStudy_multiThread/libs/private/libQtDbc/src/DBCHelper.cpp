...
QString DBCHelper::getAttributeValueString(const Vector::DBC::Signal *pSignal,
        const Vector::DBC::Network *pNetwork,
        const QString name, bool *ok)
{
    const Vector::DBC::Attribute *pAttr;
    ...
    if ((pAttr = getAttributeByName(name, pSignal->attributeValues))) {
        if (pAttr->valueType == Vector::DBC::AttributeValueType::String) {
            if (ok) *ok = true;
            return pAttr->stringValue;
        }
    }
    if (ok) *ok = false;
...
QList<QPair<quint32, QString>> DBCHelper::getSignalDescriptionPairs(const Vector::DBC::Signal *pSignal)
{
    QList<QPair<quint32, QString>> sl;
    ...
    if (!pSignal->valueDescriptions.empty())
    {
        for (auto ci = pSignal->valueDescriptions.constBegin();
                ...
                ci++)
        {
            QPair<quint64, QString> pair(ci.key(), ci.value());
            sl.append(pair);
        }
    }
    return sl;
...
int DBCHelper::getMessageDelayTime(const Vector::DBC::Message *pMessage,
		...
		const Vector::DBC::Network *pNetwork, const quint8 *data)
{
    ...
    msgGenMsgCycleTime = DBCHelper::getAttributeValueInt(
            pMessage,
            pNetwork,
            "CycleTime",
            &ok
...
int DBCHelper::getAttributeValueEnumIndex(const Vector::DBC::Message *pMessage,
        const Vector::DBC::Network *pNetwork,
        const QString name)
{
    const Vector::DBC::Attribute *pAttr;
    ...
    if (ok) {
        return pAttr->enumValue;
    }

    return -1;
}

int DBCHelper::getMessageForamt(const Vector::DBC::Message *pMessage,
                                const Vector::DBC::Network *pNetwork)
{
    if (pMessage == NULL)
        return eStandardCAN;

    int formatIndex = getAttributeValueEnumIndex(
                pMessage,
                pNetwork,
                QLatin1String("VFrameFormat")
                );

    if ((formatIndex != eExtendedCAN) &&
            (formatIndex != eStandardCAN_FD) &&
            (formatIndex != eExtendedCAN_FD)) {
        return eStandardCAN;
    }
...
bool DBCHelper::isMessageForamtFD(const Vector::DBC::Message *pMessage,
                                  const Vector::DBC::Network *pNetwork)
{
    int formatIndex = getMessageForamt(pMessage, pNetwork);
    if ((formatIndex == eStandardCAN_FD) ||
            (formatIndex == eExtendedCAN_FD)) {
        return true;
    }
