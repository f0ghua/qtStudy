...
CDDDbSTRDEFImpl::CDDDbSTRDEFImpl()
{
}
...
void CDDDbSTRDEFImpl::load(const QDomElement &element)
{
    ...
    while (!child.isNull()) {
        if (child.isElement()) {
            ...
            if (elementName == "DESC") {
                ...
                m_elString = QSharedPointer<CDDDbSTRING>::create();
                if (m_elString) {
                    m_elString->load(childElement);
                }
            }
