...
CalculatorDialog::CalculatorDialog(QWidget* parent) : QDialog(parent), ui(new Ui::CalculatorDialog)
{
    ...
    ui->txtBin->setInputMask(QString("bbbb ").repeated(sizeof(quint32) * 2));
    ...
    connect(mValidateThread, SIGNAL(expressionChanged(bool, bool, qint32)), this, SLOT(expressionChanged(bool, bool, qint32)));
...
void CalculatorDialog::expressionChanged(bool validExpression, bool validPointer, qint32 value)
{
    if(!validExpression)
    {
        ui->txtBin->setText("");
...
QString CalculatorDialog::inFormat(const quint32 val, CalculatorDialog::NUMBERFORMAT NF) const
{
    switch(NF)
    {
    default:
    ...
    case N_SDEC:
        return QString("%1").arg((qint32)val);
    case N_UDEC:
        return QString("%1").arg(val);
    case N_BIN:
    {
        QString binary = QString("%1").arg(val, 8 * sizeof(quint32), 2, QChar('0')).toUpper();
        QString ans = "";
...
{
    bool ok = false;
    qint64 val = arg1.toULongLong(&ok, 16);
    if(!ok)
    {
...
{
    bool ok = false;
    qint64 val = arg1.toLongLong(&ok, 10);
    if(!ok)
    {
...
{
    bool ok = false;
    qint64 val = arg1.toULongLong(&ok, 10);
    if(!ok)
    {
        ui->txtUnsignedDec->setStyleSheet("border: 2px solid red");
        return;
...
{
    bool ok = false;
    qint64 val = arg1.toULongLong(&ok, 8);
    if(!ok)
    {
...
void CalculatorDialog::on_txtBin_textEdited(const QString & arg1)
{
    ...
    QString text = arg1;
    text = text.replace(" ", "");
    qint64 val = text.toULongLong(&ok, 2);
    if(!ok)
    {
